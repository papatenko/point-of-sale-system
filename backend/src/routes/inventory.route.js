...
export function registerInventoryRoutes(router) {
  ...
  router.post("/api/inventory/use-menu-item", async (body, db) => InventoryService.useMenuItem(db, body));
  router.post("/api/inventory/use-daily-production", async (body, db) => InventoryService.useDailyProduction(db, body));
  router.post("/api/inventory/expire", async (body, db) => InventoryService.expireInventory(db, body));
  ...
  router.get("/api/inventory/today-sales", async (body, db, url) => InventoryService.getTodaysSales(db, url));
}
